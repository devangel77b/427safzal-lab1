...
We conducted this experiment using a meter stick, a few pieces of tape, a feather weighing \qty{0.025}{\gram}, a penny weighing \qty{0.050}{\gram}, a transparent acrylic tube (inner diameter \qty{8.00}{\centi\meter}), connected to a laboratory vacuum pump via a valve system to reduce air density inside the tube. We had previously decided that we were going to film our experiment in order to capture our data accordingly, which was done on an iPhone 14 at \qty{240}{frame\per\second}. The camera was mounted stationary and perpendicular to the tube to minimize perspective distortion. The high fps ensured that the objects in fall would be captured throughout, reducing human reaction error and allowing us to collect more accurate position data. To minimize other data collection errors from the video and because the tube had to be flipped, causing unsteadiness, we taped our meter stick to the tube. This allowed us not to have to limit our range of motion and could flip the tube easily, all while still being able to capture a measurement of position for each of the objects in the video. To obtain the positions of each object in the presence of air resistance, the tube was screwed tight, containing both the penny and the feather, and an open valve that allowed air inside of it. Before we started to film, the tube had already been flipped upside down, and we filmed and used the data of it being flipped right side up for a total of five trials in order to average out the positions captured with the aim to minimize confounding variables that one trial may have. To create the air-resistance-free environment, we connected the vacuum to the tube with a pipe and waited for the air to be sucked out, which took around ten minutes, and then we shut the valve. We had actually performed a test trial for this and found that air was slowly seeping its way back into the tube, so we had to keep the tube connected and the valve closed to ensure no air would come inside. A similar methodology was used when actually collecting the data for the air-resistance free environment, where we also collected data for a total of five trials. We then timestamped the videos and created tables (separate) of the position of the penny and feather every \qty{0.25}{\second} for each trial of each object, then averaged the results of all the trials. Finally, we created two graphs: one graph representing the position vs time of a penny and a penny with air resistance present and one graph representing the same with the same objects without air resistance present. To confirm our hypothesis, average velocity was calculated using the finite difference equation:
\begin{equation}
...
\end{equation}
The speeds and times were then put in a table to determine whether the velocity changed over time, which would support or refute our hypothesis. A least squares regression line (LSRL) and an $R^2$ value, the percentage of data accounted for by the LSRL, were also obtained. Although the video was recorded at \qty{240}{frame\per\second}, position data were sampled at \qty{0.25}{\second} intervals to reduce digitization noise; this choice significantly reduced the temporal resolution of the measurements.
